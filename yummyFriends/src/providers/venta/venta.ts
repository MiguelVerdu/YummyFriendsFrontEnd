...
export class VentaProvider {
  ...
  getTotalVenta(idVenta: number): Observable<number> {
    return this.http.get<number>(this.vagl.ip + "totalVentaCompra/"+ idVenta)
  }
  ...
  getVentasRealizadas(idUsuario: number): Observable<Venta[]> {
    return this.http.get<Venta[]>(
      this.vagl.ip + "ventasRealizadas/" + idUsuario
    );
  }

  getVentasEnPublicacion(idUsuario: number): Observable<Venta[]> {
    return this.http.get<Venta[]>(
      this.vagl.ip + "ventasEnPublicacion/" + idUsuario
    );
  }
}
