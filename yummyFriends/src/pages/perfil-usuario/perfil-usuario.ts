...
export class PerfilUsuarioPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public PerfilUsuario: PerfilUsuarioProvider,
    public formBuilder: FormBuilder, public ciudadProv: CiudadProvider, public datepipe: DatePipe
  ...
  ionViewDidLoad() {
    ...
    this.PerfilUsuario.getUsuario(this.usuario.idUsuario).subscribe((data) => {
      this.usuario = data;
      ...
      // this.date = new Date();
      this.date = this.datepipe.transform(this.usuario.fechaNac, 'dd-MM-yyyy');
      console.log("aux: " + this.date);

      let aux = new Date(this.date.replace(/(\d{2})-(\d{2})-(\d{4})/, "$2/$1/$3"));
    ...
    this.ciudadProv.getCiudades().subscribe((data) => {
      this.ciudades = data;
    },
      (error: any) => {
        console.log(error);
  ...
  actualizar() {
    ...
    if (form.password != "") {
      let usuario: Usuario = new Usuario(form.name, form.lastName, form.dateBirth,
        form.ciudad, null, form.password, form.tipo, null, form.email, this.usuario.idUsuario);

      this.PerfilUsuario.actualizarUsuario(usuario).subscribe((data) => {
        if (data != null) {
